/*
    ...
    -------정규표현식-------
    정규표현식이란: 특정한 규칙을 가진 문자열의 집합을 표현할때 사용하는 형식 언어
        -주로 문자열 데이터를 검사할때 사용 -유효성 검사.
        -메소드
        ...
        -형식규칙
            ...
            *   : 앞에 있는 패턴 0개 반복
            ...
            예2 ) /^[A-Za-z0-9]{5,30}$/
                영 대소문자와 숫자 조합의 5~30글자 허용
            예3 ) 1개 이상 필수
                (?=.*[A-Za-z]) (?=.*[0-9])[A-Za-z0-9]{5,30}
                영 대소문자 1개 이상 필수, 숫자 1개 이상 필수
            예4 ) /^[가-힣]{5,20}$/
                한글 5~20글자
            예5 ) 000-0000-000 또는 00-0000-0000
...
// 이름 유효성검사 : 한글 2~20
function namecheck(){
    let name = document.querySelector('#name').value;   //1. 입력값 가져오기
    let 이름규칙 =/^[가-힣]{2,20}$/
    let msg='';
    checkArray[2]= false;
    if(이름규칙.test(name)){
        msg = '통과'; checkArray[2] = true;
    }
    else{msg= '한글 2~20글자'}
    document.querySelector('.namecheckbox').innerHTML = msg;
}
// 전화번호 유효성검사  000-0000-0000 00-0000-0000
function phonecheck(){
...
function onChangeImg(e){
    console.log('preimg');
    console.log(e);             // 현재 함수를 실행한 input
    console.log(e.files);       // 현재 input의 첨부파일들
    console.log(e.files[0]);    // 첨부파일들 중에서 첫번째 파일
    ...
    console.log(e2.target.result);  // 여기에 읽어온 첨부파일 바이트
    document.querySelector('#preimg').src = e2.target.result
    }
}// end
/*
    함수 정의 방법
