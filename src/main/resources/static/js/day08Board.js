...
function doUpdate( bno ){
    ...
    //1. 수정할 내용을 가져온다.
    let bcontent= prompt('수정할 내용:'); console.log(bcontent);
    let bpassword= prompt('비밀번호 입력:'); console.log(bpassword);
    // ----------- AJAX ------------- //
    $.ajax({
       url : '/board/update',
       method : 'POST',
       ...
       success : function ( result ){ console.log(result);
       // 4. 결과
            if( result ){  alert('수정 성공'); doRead();} // 안내후 게시물목록 새로고침
            else{    alert('수정 실패');    }
       }
    })
    // ------------------------------ //
